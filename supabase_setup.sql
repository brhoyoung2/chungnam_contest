...
-- 8) 지도교사 이메일 명단 확인 (접수 폼의 '지도교사 이메일 확인' 버튼) ---
--    ⚠️ 명단(교사 이메일)은 개인정보라 공개 레포엔 넣지 않습니다.
--       실제 명단 INSERT 는 별도 파일(_teacher_seed.sql, gitignore)로 실행하세요.
--    아래는 테이블/RPC 정의만 (데이터 제외).
create extension if not exists pg_trgm with schema extensions;
create table if not exists public."충남콘테스트_교사메일"(email text primary key);
alter table public."충남콘테스트_교사메일" enable row level security;   -- anon 직접 조회 차단(RPC로만)

create or replace function public."충남콘테스트_교사확인"(p_email text)
returns table(status text, suggestion text)
language plpgsql security definer set search_path = public, extensions as $$
declare v text := lower(btrim(p_email)); best text; sim real;
begin
  if v = '' or position('@' in v) = 0 then return query select 'none'::text, null::text; return; end if;
  if exists(select 1 from public."충남콘테스트_교사메일" where email = v) then
    return query select 'ok'::text, null::text; return;
  end if;
  select email, similarity(email, v) into best, sim
    from public."충남콘테스트_교사메일" order by similarity(email, v) desc limit 1;
  if best is not null and sim >= 0.6 then
    return query select 'typo'::text, best; return;
  end if;
  return query select 'none'::text, null::text;
end $$;
grant execute on function public."충남콘테스트_교사확인"(text) to anon;
